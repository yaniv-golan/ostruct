...
ostruct run "$DIR/templates/summary.j2" "$DIR/schemas/summary.json" \
  --model gpt-4.1 \
  -V topic="$TOPIC" \
...
if source "$DIR/../../../scripts/install/dependencies/ensure_mermaid.sh"; then
  ...
  "$DIR/scripts/json2mermaid.sh" "$TRANSCRIPT" "$TOPIC" | npx --yes @mermaid-js/mermaid-cli@latest -i - -o "$DIR/output/debate_overview.svg"
else
  echo "⚠️  Mermaid CLI not available, skipping overview diagram"
  echo "   You can manually generate it later with:"
  echo "   ./scripts/json2mermaid.sh output/debate_init.json | npx --yes @mermaid-js/mermaid-cli@latest -i - -o output/debate_overview.svg"
fi
