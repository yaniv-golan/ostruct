...
def test_case_manager_missing_path():
...
def test_case_manager_thread_safety():
    """Test thread safety of case management."""
    def worker(path: Path, original: str):
...
def test_case_manager_overwrite():
...
def test_case_manager_clear():
    ...
    assert CaseManager.get_original_case(Path("/tmp/file1.txt")) == str(Path("/tmp/file1.txt"))
    assert CaseManager.get_original_case(Path("/tmp/file2.txt")) == str(Path("/tmp/file2.txt"))


def test_case_manager_invalid_input():
    """Test handling of invalid input."""
    # Test with None path
    with pytest.raises(TypeError):
        CaseManager.set_original_case(None, "/TMP/File.txt")

    with pytest.raises(TypeError):
        CaseManager.get_original_case(None)
