...
def test_validate_windows_path():
    ...
    assert validate_windows_path(r"\\?\C:\file.txt") == "Device paths not allowed"
    assert validate_windows_path(r"\\.\C:\file.txt") == "Device paths not allowed"

    # Drive-relative paths without slash should be rejected
    assert validate_windows_path(r"C:file.txt") == "Drive-relative paths must include separator"
    ...
    assert validate_windows_path("PRN.txt") == "Windows reserved names not allowed"
    assert validate_windows_path("COM1") == "Windows reserved names not allowed"
    assert validate_windows_path("normal.txt") is None  # Valid name

    # ADS should be rejected
    assert validate_windows_path(r"file.txt:stream") == "Alternate Data Streams not allowed"
    ...
    assert validate_windows_path(r"\\server\share\file.txt") is None  # Valid UNC
