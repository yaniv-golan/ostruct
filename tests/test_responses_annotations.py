...
def _ci_call(prompt: str, model: str):
...
def test_no_markdown_no_annotation(model):
    """Test that without markdown links, no annotations are created."""
    resp = _ci_call(
        "Write 'hi' to hello.txt via Python; do NOT mention file.", model
    )
...
@pytest.mark.live
@pytest.mark.parametrize("model", MODELS)
def test_markdown_triggers_annotation(model):
...
@pytest.mark.live
@pytest.mark.parametrize("model", MODELS)
def test_file_ids_deprecated(model):
    """Test that message.file_ids field is empty in Responses API."""
    resp = _ci_call("Make a linked hello.txt", model)
