...
class TestPerformanceBaselines:
    ...
    async def test_basic_response_time(self):
        ...
        min_time, max_time = self.benchmark.EXPECTED_RESPONSE_TIMES["basic_responses"]
        
        # Allow for test environment overhead
        assert duration < max_time * 2, f"Basic response took {duration:.2f}s, expected < {max_time * 2}s"
    ...
    async def test_structured_output_performance(self):
        ...
        min_time, max_time = self.benchmark.EXPECTED_RESPONSE_TIMES["structured_output"]
        assert duration < max_time * 2, f"Structured output took {duration:.2f}s, expected < {max_time * 2}s"
    ...
    async def test_code_interpreter_performance(self):
        ...
        with patch('openai.AsyncOpenAI') as mock_openai:
            ...
            # Mock execution
            mock_response = Mock()
            mock_response.choices = [Mock(message=Mock(content='{"output": "Analysis complete"}'))]
            mock_client.chat.completions.create.return_value = mock_response
            mock_openai.return_value = mock_client
        ...
        min_time, max_time = self.benchmark.EXPECTED_RESPONSE_TIMES["code_interpreter"]
        assert duration < max_time * 2, f"Code Interpreter took {duration:.2f}s, expected < {max_time * 2}s"
    ...
    async def test_file_search_performance(self):
        ...
        with patch('openai.AsyncOpenAI') as mock_openai:
            ...
            mock_response = Mock()
            mock_response.choices = [Mock(message=Mock(content='{"results": ["found"]}'))]
            mock_client.chat.completions.create.return_value = mock_response
            mock_openai.return_value = mock_client
        ...
        min_time, max_time = self.benchmark.EXPECTED_RESPONSE_TIMES["file_search"]
        assert duration < max_time * 2, f"File Search took {duration:.2f}s, expected < {max_time * 2}s"
    ...
    async def test_mcp_integration_performance(self):
        ...
        with patch('httpx.AsyncClient') as mock_httpx:
            ...
            mock_response.json.return_value = {"result": {"content": "MCP response"}}
        ...
        min_time, max_time = self.benchmark.EXPECTED_RESPONSE_TIMES["mcp_integration"]
        assert duration < max_time * 2, f"MCP integration took {duration:.2f}s, expected < {max_time * 2}s"
    ...
    def test_token_usage_baselines(self):
        ...
        for size_name, content, baseline_key in test_cases:
            ...
            min_tokens, max_tokens = self.benchmark.EXPECTED_TOKEN_USAGE[baseline_key]
            
            # Allow some variance for test content
            assert estimated_tokens <= max_tokens * 2, \
    ...
    def test_cost_estimation_baselines(self):
        ...
        for scenario_name, token_count, baseline_key in test_scenarios:
            ...
            assert estimated_cost <= max_cost * 2, \
...
class TestPerformanceRegression:
    ...
    def test_template_rendering_performance(self):
        ...
        assert duration < 0.1, f"Template rendering took {duration:.4f}s, expected < 0.1s"
    ...
    def test_file_processing_performance(self, security_manager, fs):
        ...
        for i in range(100):
            fs.create_file(f'/test_workspace/base/file_{i}.txt', contents=f'content_{i}')
        
        start_time = time.time()
        
        # Create test files
        test_files = [
            FileInfo(path=f'/test_workspace/base/file_{i}.txt', security_manager=security_manager)
            for i in range(100)
        ]
        ...
        assert duration < 1.0, f"Processing 100 files took {duration:.4f}s, expected < 1.0s"
    ...
    def test_configuration_loading_performance(self):
        ...
        assert duration < 0.01, f"Configuration loading took {duration:.4f}s, expected < 0.01s"
    ...
    def test_security_validation_performance(self):
        ...
        security_manager = SecurityManager(base_dir="/tmp", allowed_dirs=["/tmp/test"])
        ...
        for path in test_paths:
            try:
                ...
                is_safe = security_manager.is_path_safe(path)
            except:
        ...
        assert duration < 0.5, f"Security validation took {duration:.4f}s, expected < 0.5s"
...
class TestScalabilityBaselines:
    ...
    def test_large_file_handling(self, security_manager, fs):
        ...
        fs.create_file('/test_workspace/base/large.txt', contents=large_content)
        
        start_time = time.time()
        
        # Test file info creation
        file_info = FileInfo(path='/test_workspace/base/large.txt', security_manager=security_manager)
        ...
        assert duration < 1.0, f"Large file handling took {duration:.4f}s, expected < 1.0s"
    ...
    def test_multiple_tool_coordination(self):
        ...
        assert duration < 0.5, f"Multi-tool coordination took {duration:.4f}s, expected < 0.5s"
    ...
    def test_concurrent_request_handling(self):
        ...
        average_time = total_time / completed_requests if completed_requests > 0 else 0
        
        # Concurrent requests should complete efficiently
        assert completed_requests == 5
        assert average_time < 0.2, f"Average request time {average_time:.4f}s, expected < 0.2s"
...
class TestStressBaselines:
    ...
    def test_high_concurrency_baseline(self):
        ...
        assert duration < 5.0, f"High concurrency test took {duration:.2f}s, expected < 5.0s"
    ...
    def test_large_schema_processing(self):
        ...
        schema_size = len(str(large_schema))
        ...
        assert duration < 0.1, f"Large schema processing took {duration:.4f}s, expected < 0.1s"