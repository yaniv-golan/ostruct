...
def test_simple_symlink(fs: FakeFilesystem):
    ...
    assert os.readlink(str(link_path)) == "/test/real.txt"  # readlink returns target


def test_nested_symlink_chain(fs: FakeFilesystem):
    ...
    print(f"real_path.exists(): {real_path.exists()}")
    print(f"os.readlink chain: {os.readlink(str(link3_path))} -> {os.readlink(str(link2_path))} -> {os.readlink(str(link1_path))}")
...
def test_symlink_loop(fs: FakeFilesystem):
    ...
    assert os.readlink(str(link1_path)) == "/test/link2.txt"  # readlink still works
    assert os.readlink(str(link2_path)) == "/test/link1.txt"
...
def test_broken_symlink(fs: FakeFilesystem):
    ...
    assert os.readlink(str(broken_path)) == "/test/nonexistent.txt"  # readlink still works


def test_symlink_to_symlink_to_nonexistent(fs: FakeFilesystem):
    ...
    print(f"link1_path.is_symlink(): {link1_path.is_symlink()}")
    print(f"target_path.exists(): {target_path.exists()}")
    print(f"os.readlink chain: {os.readlink(str(link2_path))} -> {os.readlink(str(link1_path))}")
    ...
    assert os.readlink(str(link2_path)) == "/test/link1.txt"  # readlink works on all links
