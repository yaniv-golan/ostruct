...
def patch_filesystem_case(fs: FakeFilesystem) -> Generator[None, None, None]:
    ...
    if sys.platform == 'darwin' or os.name == 'nt':
        ...
        fs.case_sensitive = False  # Ensure both properties are set for compatibility
...
def test_symlink_security(fs: FakeFilesystem) -> None:
    ...
    fs.create_symlink("/test_workspace/base/link", "/test_workspace/outside/target.txt")
...
def test_allowed_paths_normalization(fs: FakeFilesystem) -> None:
    ...
    test_paths = [
        ...
        ("/test_workspace/base/FILE2.txt", os.name != 'posix' or sys.platform == 'darwin'),
        ("/test_workspace/allowed/file3.txt", True),
        ("/test_workspace/allowed/FILE4.txt", os.name != 'posix' or sys.platform == 'darwin'),
    ]
