...
class FileSearchManager:
    ...
    async def create_vector_store_with_retry(
        ...
        for attempt in range(max_retries + 1):
            try:
                logger.debug(f"Creating vector store '{name}' (attempt {attempt + 1}/{max_retries + 1})")
                ...
                logger.debug(f"Successfully created vector store: {vector_store.id}")
                return vector_store.id
                
            except Exception as e:
                last_exception = e
                logger.warning(f"Vector store creation attempt {attempt + 1} failed: {e}")
                ...
                if attempt < max_retries:
                    ...
                    logger.error(f"Vector store creation failed after {max_retries + 1} attempts")
        
        raise Exception(f"Failed to create vector store after {max_retries + 1} attempts: {last_exception}")
    ...
    async def _upload_single_file_with_retry(
        ...
        for attempt in range(max_retries + 1):
            try:
                logger.debug(f"Uploading {file_path} ({file_size} bytes) - attempt {attempt + 1}/{max_retries + 1}")
                ...
                last_exception = e
                logger.warning(f"File upload attempt {attempt + 1} failed for {file_path}: {e}")
                ...
                if attempt < max_retries:
                    ...
                    logger.error(f"File upload failed after {max_retries + 1} attempts: {file_path}")
        
        raise Exception(f"Failed to upload {file_path} after {max_retries + 1} attempts: {last_exception}")
    ...
    async def _add_files_to_vector_store_with_retry(
        ...
        for attempt in range(max_retries + 1):
            try:
                logger.debug(f"Adding {len(file_ids)} files to vector store - attempt {attempt + 1}/{max_retries + 1}")
                ...
                logger.debug(f"Successfully added files to vector store: {vector_store_id}")
                return
                
            except Exception as e:
                last_exception = e
                logger.warning(f"Adding files to vector store attempt {attempt + 1} failed: {e}")
                ...
                if attempt < max_retries:
                    ...
                    logger.error(f"Adding files to vector store failed after {max_retries + 1} attempts")
        
        raise Exception(f"Failed to add files to vector store after {max_retries + 1} attempts: {last_exception}")
    ...
    async def wait_for_vector_store_ready(
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                if vector_store.status == "completed":
                    ...
                    logger.error(f"Vector store {vector_store_id} failed to index")
                    return False
                
                logger.debug(f"Vector store {vector_store_id} status: {vector_store.status}")
        ...
        logger.warning(f"Vector store {vector_store_id} not ready after {timeout}s timeout")
        return False
    ...
    async def _cleanup_vector_stores(self, vector_store_ids: List[str]) -> None:
        ...
        for file_path in files:
            try:
                ...
                if file_ext not in supported_extensions:
                    logger.warning(f"File extension {file_ext} may not be optimal for File Search: {file_path}")
    ...
    def get_performance_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "supported_formats": ["PDF", "text files", "documents", "code files"],
