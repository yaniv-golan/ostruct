...
class ProgressContext:
    ...
    def __init__(
        ...
        logger.debug("Initializing ProgressContext with level=%s, output_file=%s", level, output_file)
        logger.debug("Description: %s, total: %s", description, total)
        self._output_file = output_file
        self._level = level
        self.enabled = level != "none"
        self.current: int = 0
        logger.debug("ProgressContext initialized with enabled=%s", self.enabled)
    ...
    def __exit__(
        ...
        logger.debug("Exiting ProgressContext. Had exception: %s", exc_type is not None)
        if exc_type:
            logger.error("Exception in ProgressContext: %s - %s", exc_type, exc_val)
        pass

    def update(self, amount: int = 1, force: bool = False) -> None:
        """No-op update method kept for compatibility."""
        logger.debug("Update called with amount=%d, force=%s, current=%d", amount, force, self.current)
        self.current += amount
        pass
    ...
    def print_output(self, text: str) -> None:
        ...
        logger.debug("Output file: %s, enabled=%s, level=%s", self._output_file, self.enabled, self._level)
        ...
        try:
            if self._output_file:
                ...
                with open(self._output_file, "a", encoding="utf-8") as f:
                    ...
                    f.write(text)
                    f.write("\n")
                    logger.debug("Successfully wrote %d chars to file", len(text))
            else:
                logger.debug("Writing to stdout")
                print(text)
                logger.debug("Successfully wrote %d chars to stdout", len(text))
