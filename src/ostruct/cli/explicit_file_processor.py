...
class ExplicitRouting:
    ...
    code_interpreter_files: List[str] = field(default_factory=list)  # Code Interpreter uploads
    ...
    code_interpreter_dirs: List[str] = field(default_factory=list)   # Code Interpreter directory uploads
    file_search_dirs: List[str] = field(default_factory=list)        # File Search directory uploads
...
class ExplicitFileProcessor:
    ...
    def _parse_file_routing_from_args(self, args: Dict[str, Any]) -> ExplicitRouting:
        ...
        if legacy_files:
            logger.debug(f"Legacy -f flag detected: {len(legacy_files)} files (handled separately)")
        
        if legacy_dirs:
            logger.debug(f"Legacy -d flag detected: {len(legacy_dirs)} dirs (handled separately)")
    ...
    async def _validate_routing_security(self, routing: ExplicitRouting) -> ExplicitRouting:
        ...
        for dir_path in all_dirs:
            try:
                ...
                logger.error(f"Security validation failed for directory {dir_path}: {e}")
                raise
        
        logger.debug(f"Security validation passed for {len(all_files)} files and {len(all_dirs)} directories")
        return routing

    def _create_validated_file_mappings(self, routing: ExplicitRouting) -> Dict[str, List[str]]:
    ...
    def validate_routing_consistency(self, routing: ExplicitRouting) -> List[str]:
        ...
        all_files = (routing.template_files + routing.code_interpreter_files + 
        ...
        all_dirs = (routing.template_dirs + routing.code_interpreter_dirs + 
        ...
        for tool, files in [
            ...
            for file_path in files:
                if file_path in file_tool_mapping:
                    issues.append(f"File {file_path} is routed to multiple tools: {file_tool_mapping[file_path]} and {tool}")
                else:
