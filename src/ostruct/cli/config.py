...
class ToolsConfig(BaseModel):
    ...
    code_interpreter: Dict[str, Any] = Field(default_factory=lambda: {
        "auto_download": True,
        "output_directory": "./output"
    })
    file_search: Dict[str, Any] = Field(default_factory=lambda: {
        "max_results": 10
    })
...
class OstructConfig(BaseModel):
    ...
    def should_warn_expensive(self, cost_estimate: float) -> bool:
        ...
        return (self.limits.warn_expensive_operations and 
                cost_estimate > self.limits.max_cost_per_run * 0.3)
