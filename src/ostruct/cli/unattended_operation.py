...
class UnattendedOperationTimeoutError(CLIError):
    ...
    def __init__(
        ...
        super().__init__(enhanced_message, exit_code=exit_code, context=enhanced_context)
...
class UnattendedOperationManager:
    ...
    def __init__(self, timeout_seconds: int = 3600):  # 1 hour default
        ...
        logger.debug(f"UnattendedOperationManager initialized with {timeout_seconds}s timeout")

    async def execute_with_safeguards(self, operation: Callable, operation_name: str = "operation") -> Any:
        ...
        """
        logger.debug(f"Starting unattended {operation_name} with {self.timeout}s timeout")
    ...
    def validate_tool_configurations(tool_configs: List[dict]) -> List[str]:
        ...
        for tool_config in tool_configs:
            ...
            # Unknown tool types - warn but don't fail
                ...
                logger.warning(f"Unknown tool type '{tool_type}' - cannot validate for unattended operation")
