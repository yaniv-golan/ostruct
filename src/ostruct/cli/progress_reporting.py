...
class EnhancedProgressReporter:
    ...
    def report_optimization(self, optimizations: List[str]) -> None:
        ...
        if self.detailed:
            ...
            click.echo(f"🔧 Optimized template for better AI performance ({len(optimizations)} improvements)", err=True)

    def report_file_routing(self, template_files: List[str], container_files: List[str], 
        ...
        if self.detailed:
            ...
            for file in template_files:
                click.echo(f"  → {file}: available in template for direct access", err=True)
            for file in container_files:
                click.echo(f"  → {file}: uploaded to Code Interpreter for analysis", err=True)
            for file in vector_files:
                click.echo(f"  → {file}: uploaded to File Search for semantic search", err=True)
        else:
            tools_used = []
            if container_files:
    ...
    def report_processing_results(self, result: ProcessingResult) -> None:
        ...
        if self.detailed:
            click.echo(f"📦 Processing results:", err=True)
    ...
    def report_cost_breakdown(self, cost_info: CostBreakdown) -> None:
        ...
        if self.detailed:
            ...
            click.echo(f"  ├── Input tokens ({cost_info.input_tokens:,}): ${cost_info.input_cost:.4f}", err=True)
            click.echo(f"  ├── Output tokens ({cost_info.output_tokens:,}): ${cost_info.output_cost:.4f}", err=True)
            if cost_info.code_interpreter_cost:
                click.echo(f"  ├── Code Interpreter: ${cost_info.code_interpreter_cost:.4f}", err=True)
            if cost_info.file_search_cost:
                click.echo(f"  └── File Search: ${cost_info.file_search_cost:.4f}", err=True)
        else:
            if cost_info.total > 0.01:  # Only show cost if significant
                if cost_info.has_tool_costs:
                    click.echo(f"💰 Total cost: ${cost_info.total:.3f} (model + tools)", err=True)
                else:
                    click.echo(f"💰 Total cost: ${cost_info.total:.3f}", err=True)

    def report_file_downloads(self, downloaded_files: List[str], download_dir: str) -> None:
        ...
        if self.detailed:
            click.echo(f"📥 Downloaded {len(downloaded_files)} generated files to {download_dir}:", err=True)
            for file_path in downloaded_files:
                click.echo(f"  → {file_path}", err=True)
        else:
            click.echo(f"📥 Downloaded {len(downloaded_files)} files to {download_dir}", err=True)

    def report_error(self, error_type: str, error_message: str, suggestions: List[str] = None) -> None:
    ...
    def report_validation_results(self, schema_valid: bool, template_valid: bool, 
                                token_count: int, token_limit: int) -> None:
        ...
        if self.detailed:
            ...
            click.echo(f"  ├── Schema: {'✅ Valid' if schema_valid else '❌ Invalid'}", err=True)
            click.echo(f"  ├── Template: {'✅ Valid' if template_valid else '❌ Invalid'}", err=True)
            click.echo(f"  └── Tokens: {token_count:,} / {token_limit:,} ({(token_count/token_limit)*100:.1f}%)", err=True)
        else:
            if schema_valid and template_valid:
                usage_pct = (token_count / token_limit) * 100
                if usage_pct > 80:
                    click.echo(f"✅ Validation passed (⚠️  {usage_pct:.0f}% token usage)", err=True)
                else:
    ...
    def _humanize_optimization(self, technical_message: str) -> str:
        ...
        if "moved" in technical_message.lower() and "appendix" in technical_message.lower():
...
def configure_progress_reporter(verbose: bool = False, progress_level: str = "basic") -> None:
