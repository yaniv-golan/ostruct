...
if [[ -f "$SCRIPT_DIR/../../scripts/logging.sh" ]]; then
  ...
  echo "❌ logging.sh not found at $SCRIPT_DIR/../../scripts/logging.sh" >&2
  exit 1
fi
...
OPTIONS:
    -o, --output FILE          Output file for the final validated schema (default: <template-name>.json)
...
EXAMPLES:
    ...
    $0 --verbose --validator-cmd jsonschema templates/extract.j2

PREREQUISITES:
    - ostruct must be installed and accessible on PATH
    - A JSON Schema validator CLI (ajv-cli recommended: npm install -g ajv-cli)
    - jq for JSON processing

For more information, see the README.md in the tools/schema-generator/ directory.
EOF
}
...
check_prerequisites() {
    ...
    # Check ostruct on PATH
    if ! command -v ostruct &> /dev/null; then
        missing_deps+=("ostruct")
    fi

    # Check jq using ensure_jq script
    if [[ -f "$SCRIPT_DIR/../../scripts/install/dependencies/ensure_jq.sh" ]]; then
        logger_debug "Using ensure_jq.sh for jq dependency check"
        if ! "$SCRIPT_DIR/../../scripts/install/dependencies/ensure_jq.sh"; then
            missing_deps+=("jq (ensure_jq.sh failed)")
        fi
    else
    ...
    if [[ ${#missing_deps[@]} -gt 0 ]]; then
        ...
        echo
        echo "For detailed instructions, see: tools/schema-generator/README.md"
        exit 1
    fi

    # In verbose/debug mode, show ostruct executable path
    if [[ "${LOG_LEVEL}" == "DEBUG" ]] || [[ "${LOG_LEVEL}" == "INFO" ]]; then
        local ostruct_path
        ostruct_path=$(command -v ostruct)
        logger_info "Using ostruct executable: $ostruct_path"
    fi
}
...
generate_schema() {
    ...
    # Build ostruct command with GPT-4.1 model
    local cmd=(
        ostruct run
        "${prompt_template}"
        "${output_schema}"
    ...
    logger_debug "Running command: ${cmd[*]}"
...
refine_schema() {
    ...
    # Build refinement command with GPT-4.1
    local cmd=(
        ostruct run
        "${BASE_DIR}/src/refine_schema_from_feedback.j2"
        "${BASE_DIR}/src/schema_generator_output.json"
        --file user_template "${target_template}"
        --model gpt-4.1
    ...
    logger_debug "Running refinement command: ${cmd[*]}"
...
main() {
    ...
    # Set default output file to <template-name>.json
    if [[ -z "$output_file" ]]; then
        local template_basename
        template_basename=$(basename "$target_template")
        template_basename="${template_basename%.*}"  # Remove extension
        output_file="${template_basename}.json"
        logger_info "Using default output file: $output_file"
    fi
    ...
    logger_debug "Using temporary directory: $TEMP_DIR"
    ...
    while [[ $retry_count -le $max_retries ]]; do
        if [[ $retry_count -eq 0 ]]; then
            ...
            if generate_schema \
                "$target_template" \
                "${BASE_DIR}/src/generate_schema_from_template.j2" \
                "${BASE_DIR}/src/schema_generator_output.json" \
                "$temp_ostruct_output" \
                ""; then
    ...
    if [[ "$success" == "true" ]]; then
        ...
        if [[ "${LOG_LEVEL}" == "DEBUG" ]]; then
            print_status "$BLUE" "Final schema content:"
            cat "$final_schema"
        fi

        log_finish "Schema Generator"
        exit 0
    else
