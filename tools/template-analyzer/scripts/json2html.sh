#!/usr/bin/env bash
# Convert template analysis JSON to interactive HTML visualization
set -euo pipefail

ANALYSIS_JSON=${1:-analysis_output.json}
OUTPUT_HTML=${2:-analysis_report.html}
...
if [ -f "$ANALYSIS_JSON" ]; then
    jq '.' "$ANALYSIS_JSON" >> "$OUTPUT_HTML"
else
    echo "null" >> "$OUTPUT_HTML"
fi
                ...
                button.addEventListener('click', () => {
                    ...
                    issues.forEach(issue => {
                        if (filter === 'all') {
                            ...
                            // Map filter values to actual severity values in data
                            const severityMap = {
                                'error': 'Critical',
                                'warning': 'Warning',
                                'optimization': 'Optimization'
                            };
                            const expectedSeverity = severityMap[filter];
                            if (issue.getAttribute('data-severity') === expectedSeverity) {
                                issue.classList.remove('hidden');
                            } else {
...
echo "Generated interactive HTML report: $OUTPUT_HTML"
