...
class ConfigMigrator:
    ...
    def migrate_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'file_routing' in migrated_config:
            ...
            migrations.append("Migrated file_routing to attachments configuration")
        ...
        if 'security' in migrated_config:
            ...
            if 'allowed_dirs' in security:
                ...
                migrated_config['attachments']['allowed_directories'] = security.pop('allowed_dirs')
                migrations.append("Moved security.allowed_dirs to attachments.allowed_directories")
            ...
            if 'base_dir' in security or 'mode' in security:
                ...
                if 'mode' in security:
                    migrated_config['path_security']['mode'] = security.pop('mode')
                else:
                    migrated_config['path_security']['mode'] = 'warn'
                
                if 'base_dir' in security:
                    base_dir = security.pop('base_dir')
                    migrated_config['path_security']['allowed_directories'] = [base_dir]
                
                migrations.append("Created path_security configuration from legacy security settings")
        ...
        if 'models' in migrated_config:
            ...
            if 'default' in models and models['default'] == 'gpt-4':
                ...
                warnings.append("Default model changed to gpt-4o - verify this works for your use case")
        else:
    ...
    def validate_config(self, config: Dict[str, Any]) -> Dict[str, List[str]]:
        ...
        if 'attachments' in config:
            ...
            if 'default_target' not in attachments:
                validation['warnings'].append("No default_target specified in attachments")
            elif attachments['default_target'] not in ['prompt', 'code-interpreter', 'file-search']:
                validation['errors'].append(f"Invalid default_target: {attachments['default_target']}")
        ...
        if 'path_security' in config:
            ...
            if 'mode' in path_security:
                ...
                if path_security['mode'] not in valid_modes:
                    validation['errors'].append(f"Invalid path_security mode: {path_security['mode']}")
        ...
        if 'tools' in config:
            ...
            if 'code_interpreter' in tools:
                ...
                if 'output_directory' in ci_config:
                    ...
                    if not output_dir.exists() and str(output_dir) != './downloads':
                        validation['warnings'].append(f"Code interpreter output directory doesn't exist: {output_dir}")
        
        # Security recommendations
        if 'path_security' not in config:
            validation['recommendations'].append("Consider adding path_security configuration for enhanced security")
        
        if config.get('path_security', {}).get('mode') == 'permissive':
            validation['recommendations'].append("Consider using 'warn' or 'strict' security mode for better security")
...
Examples:
  ...
  %(prog)s ostruct.yaml --analyze     # Only analyze without migrating
    ...
    parser.add_argument('files', nargs='+', help='Configuration files to migrate')
        ...
        for config_path in Path.cwd().glob(file_pattern):
            if config_path.is_file() and config_path.suffix in ['.yaml', '.yml']:
                ...
                if analysis['legacy_sections']:
                    print(f"🔄 Legacy sections found: {', '.join(analysis['legacy_sections'])}")
                
                if analysis['missing_sections']:
                    print(f"➕ Missing v0.9.0 sections: {', '.join(analysis['missing_sections'])}")
